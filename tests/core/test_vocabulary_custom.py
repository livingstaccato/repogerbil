...
def test_vocabulary_hierarchy() -> None:
    """Vocabulary should support hierarchical category associations."""
    vocab = VocabularyConfig(
        categories={
            "standard": CategoryDefinition(label="std", conventional="feat"),
            "custom": CategoryDefinition(label="cust", conventional="fix", parent="standard"),
        }
    )
    
    assert get_root_category("custom", vocab=vocab) == "standard"
    assert get_root_category("standard", vocab=vocab) == "standard"
    assert get_root_category("nonexistent", vocab=vocab) == "nonexistent"
