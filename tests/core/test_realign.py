...
def _commit(
    ...
    subprocess.run(
        ["git", "commit", "-q", "-m", message], cwd=repo, env=env, capture_output=True, check=True
    )
...
class TestRealignBasic:
    def test_already_verified_unchanged(self, tmp_path: Path) -> None:
        ...
        _write_jsonl(jsonl, [{
            "hash": sha, "date": "2026-04-20",
            "subjects": ["feat: one"], "body": "", "changes": [{"file": "a.txt", "description": ""}]
        }])
...
class TestRealignPicking:
    ...
    def test_mixed_records(self, tmp_path: Path) -> None:
        ...
        jsonl = tmp_path / "s.jsonl"
        _write_jsonl(jsonl, [
            {"hash": sha1, "date": "2025-10-15", "subjects": [], "body": "",
             "changes": [{"file": "a.txt", "description": ""}]},
            {"hash": "f" * 40, "date": "2025-10-16", "subjects": [], "body": "",
             "changes": [{"file": "b.txt", "description": ""}]},
            {"hash": "e" * 40, "date": "1999-01-01", "subjects": [], "body": "",
