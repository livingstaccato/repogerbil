...
from repogerbil.core.config import FileRule, VocabularyConfig
...
def test_classify_conventional_edge_cases() -> None:
    ...
    assert res.severity is None  # errata maps to None
    
    # 4. prefix NOT in _PREFIX_SEVERITY
    res = _try_conventional_prefix("feat: message", "", True)
    assert res.category == "instantiate"
    assert res.severity == "minor" # behavioral maps to minor
